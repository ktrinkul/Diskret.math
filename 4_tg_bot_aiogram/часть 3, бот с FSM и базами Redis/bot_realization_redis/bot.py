...
from aiogram.fsm.storage.memory import RedisStorage, Redis
...
async def send_echo(message: types.Message):
    await message.answer('Извините, такой команды нет\n'
                         'Для возвращения в начало отправьте команду /start')

@dp.message(FSMtrips.get_classic_prose_for)
async def send_echo(message: types.Message):
    await message.answer('То, что вы отправили, не похоже на предоженные варинаты\n'
                         'Если вы больше не хотите классический стих -'
